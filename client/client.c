...
int main(int argc, char *argv[])
{
    ...
    while (client_data.current_state != STATE_EXIT)
    {
        ...
        else if (current_state == STATE_WRITE_BIO)
        {
            ...
            receive_and_deserialize_Response(socket_fd, 10); // Ignore response for now
        ...
        else if (current_state == STATE_LOGIN)
        {
            ...
            if (fgets(input, sizeof(input), stdin))
            {
                ...
                if (strcmp(input, "yes") == 0)
                {
                    ...
                    Response *response = send_command_and_receive_response(socket_fd, cmd, 5000);
                    free(args[0]);
                    free(args[1]);
        ...
        else if (current_state == STATE_VIEW_CHALLENGES)
        {
            ...
            if (response != NULL)
            {
                if (response->status_code == 1 && response->body_size > 0)
                {
                    ...
                    // Verifies if the chosen user is in the incoming challenges
                    bool found = false;
                    ...
                    {
                        // Send ACCEPT_CHALLENGE command
                        char *args[1] = {client_data.selected_chat_user};
                        Command *accept_cmd = createCommand("ACCEPT_CHALLENGE", args, 1);
                        Response *accept_response = send_command_and_receive_response(socket_fd, accept_cmd, 5000);

                        if (accept_response != NULL && accept_response->status_code == 1)
                        {
                            snprintf(client_data.status_message, sizeof(client_data.status_message),
                        ...
                        if (accept_response)
                        {
                            for (int i = 0; i < accept_response->body_size; i++)
                                free(accept_response->body[i]);
        ...
        else if (current_state == STATE_CHALLENGE)
        {
            ...
            if (activity > 0 && FD_ISSET(socket_fd, &readfds))
            {
                ...
                Response *response = receive_and_deserialize_Response(socket_fd, 0);
                pthread_mutex_lock(&client_data.data_mutex);
        ...
        else if (current_state == STATE_IN_GAME)
        {
            ...
            while (!game_over)
            {
                ...
                while (!game_over)
                {
                    ...
                    Response *update_response = send_command_and_receive_response(socket_fd, update_cmd, 5000);
                    ...
                    if (update_response->status_code == 1 && update_response->body_size > 0)
                    {
                        ...
                        // If it's the client's turn, break the waiting loop
                ...
                Response *move_response = send_command_and_receive_response(socket_fd, move_cmd, 5000);
        ...
        else if (current_state == STATE_CHOOSE_GAME_SPECTATE)
        {
            ...
            else
            {
                ...
                Response *spectate_response = send_command_and_receive_response(socket_fd, spectate_cmd, 5000);
                if (spectate_response != NULL)
                {
                    if (response->status_code == 1)
                    {
                        snprintf(client_data.status_message, sizeof(client_data.status_message),
        ...
        else if (current_state == STATE_SPECTATING)
        {
            ...
            Response *response = send_command_and_receive_response(socket_fd, update_cmd, 5000);
        ...
        else if (current_state == STATE_REVIEWING)
        {
            ...
            while (1)
            {
                ...
                if (strcmp(command, "next") == 0 || strcmp(command, "prev") == 0)
                {
                    ...
                    Response *nav_resp = receive_and_deserialize_Response(socket_fd, 0);
                    ...
                    for (int i = 0; i < nav_resp->body_size; i++)
                        free(nav_resp->body[i]);
                    free(nav_resp);
                }
            }
        }
